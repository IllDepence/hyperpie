...
text_e2e_fillin_twostep_1_wizardlm = f"""A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions. USER: In the context of machine learning and related fields, what are the entities (allowed types: datasets, models, methods, loss functions, regularization techniques) mentioned in the LaTeX Input Text below? What are their parameters and values?
...
text_e2e_fillin_twostep_1_falcon = f"""User: I will give you instructions consisting of three parts. The three parts are called TASK, FORMAT and TEXT. You have to perform TASK based solely on information in TEXT, and in your output adhering to FORMAT.
