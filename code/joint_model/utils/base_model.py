...
# self define
from utils.util import process_long_input
...
class Encoder(nn.Module):
    ...
    def forward(self,
        ...
        entity_type_index = ['a', 'p', 'v', 'c']
        # relation_index = ['pa', 'vp', 'cv','nan']
        relation_index = ['pa', 'vp', 'cv']
        ...
        for batch_i in range(sequence_output.size(0)):
            ...
            if relation_labels != None:
                ...
                # labels
                labels = torch.zeros((target_shape[0], target_shape[0], 4)).to(sequence_output)
                labels[:,:,0] = 1.0
